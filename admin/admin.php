...
<body>
  ...
  <div class="container">
    <div class="row">
      ...
      <?php
      $con1=mysqli_connect("localhost","root","","airlinebooking");
      $que1="select * from admin where emailid='$id'";
      ...
      echo '
      <div class="col-md-9">
        <form method="POST" action="admindp.php" enctype="multipart/form-data">
          <input type="file" name="dp" id="file"/>
          <center><img style="margin-top: 15px;width:200px;" class="rounded-circle" src="../user/dp/'.$dp.'"></img></center>
          <button style="margin-left:45%; margin-top:3px;" type="submit" class="btn btn-info">change</button>
        </form>
        <form method="POST" action="updateadmin.php">
          <div class="f" style="margin-top:30px;">
            ...
            <div class="form-group">
              <label for="usr"></label>
              <input type="password" class="form-control" id="pass" name="pass" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" placeholder="CHANGE PASSWORD" value="'.$pass.'"><input type="checkbox" onclick="myFunction()"><p style="color:white;">Show password</p></div>
            </div>
